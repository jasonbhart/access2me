...
class SenderAuthHandler extends Auth\AbstractHandler
{
    ...
    public function handle($linkedinRequest, $linkedinResponse)
    {
        ...
        $message = $mesgRepo->getById($linkedinRequest->messageId);

        if (!$message) {
            \Logging::getLogger()->error('Invalid messageId: ' . $linkedinRequest->messageId);
            return;
        }
        ...
        $profile = $defaultProfileProvider->getProfile($senders, Service\Service::LINKEDIN);
