...
foreach ($messages AS $message) {
    ...
    if ($filter->status === true) {
        ...
        $mail->FromName = $contact['first_name'] . " " . $contact['last_name'] . " (via Access2.ME)";
        $mail->addAddress($to[0]['email']);
        $mail->addReplyTo($message['from_email']);
        $mail->XMailer = ' ';
